...
*/
public class CompanyHomepage extends javax.swing.JDialog {
...
public CompanyHomepage(Vector items, Vector names) {
	...
	public CompanyHomepage(JFrame frame) {
		super(frame);
		initGUI();
	}
	...
	private void initGUI() {
		try{
			POIFSFileSystem fs =
					new POIFSFileSystem(new FileInputStream("ticketinfo.xls"));
			HSSFWorkbook wb = new HSSFWorkbook(fs);
			HSSFSheet sheet1 = wb.getSheet("Tickets");
			...
			for (int i=0; i<items1.size(); i++)
			{
				...
				Container.add(item);
				NameContainer.add(item.getEvent());
		...
		try {
			{
				...
				}
				{
					...
					title.setText("Company Customer");
				...
				}
				{
					...
					viewer.addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent evt) {
							...
							TicketViewer tv = new TicketViewer(Container, NameContainer);
							tv.show();
							dispose();
						}
					});
				}
				{
					...
					jButton1.setText("Post New Ticket");
					...
					jButton1.addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent evt) {
							...
							TicketPost tp = new TicketPost(Container, NameContainer);
							tp.show();
							dispose();
						}
					});
